...
cleanly. Per-process, runtime, reversible — not a global on-disk DLL swap. Covers fourteen functions across
`ucrtbase` and `ntdll` — the compares, a transform (`RtlUpcaseUnicodeString`), the integer formatters, — a transform that writes an upcased output
