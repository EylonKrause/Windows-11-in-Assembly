...
| [154](changes/154-strrev/) | `_strrev` (reverse in place) | `ucrtbase.dll!_strrev` | **LANDED** — **7.21×** (up to **32.68×**; two-ended 32-byte block swap via `vpshufb`+`vperm2i128`, with the final block pair deliberately allowed to overlap — both stores provably write the same bytes there) |
| [155](changes/155-wcsrev/) | `_wcsrev` (wide reverse in place) | `ucrtbase.dll!_wcsrev` | **LANDED** — **6.73×** (up to **19.45×**; wide port of 154 — `vpcmpeqw` makes the length a byte count, so the block loop, the overlapping final pair and the centred middle are the same code) |
| [156](changes/156-wcsspn/) | `wcsspn` (span of set members) | `ucrtbase.dll!wcsspn` | **LANDED** — **9.10×** (up to **29.91×**; the wide half left scalar because 65536 values defeat the byte bitmap `strspn` uses. Set hoisted into registers once, spare slots taking a duplicate of member 0 since `a OR a == a`) |
| [157](changes/157-wcscspn/) | `wcscspn` (complement span) | `ucrtbase.dll!wcscspn` | **LANDED** — **9.12×** (up to **29.40×**; hoisting also removed a code-layout hazard — three added µops moved the 1024-char result between 96 and 125 ns with no change in work done) |
| [158](changes/158-wcspbrk/) | `wcspbrk` (first set member) | `ucrtbase.dll!wcspbrk` | **LANDED** — **9.08×** (up to **22.06×**; hits and terminator kept in separate masks, `bt` decides which stop came first) |
| [159](changes/159-strspn/) | `strspn` (span of set members) | `ucrtbase.dll!strspn` | **LANDED** — **8.60×** (up to **27.14×**; unlike the wide version this one already had a 256-bit set bitmap, so the win is 32 chars per step instead of one, plus no table build at all for a small set) |
| [160](changes/160-strcspn/) | `strcspn` (complement span) | `ucrtbase.dll!strcspn` | **LANDED** — **5.81×** (up to **21.57×**; byte-granular twin of 157 — the NUL must be OR-ed into the stop mask, which the plain span gets for free) |
| [161](changes/161-strpbrk/) | `strpbrk` (first set member) | `ucrtbase.dll!strpbrk` | **LANDED** — **5.75×** (up to **19.43×**; hits and terminator in separate masks so a member one byte past the terminator still returns NULL) |
