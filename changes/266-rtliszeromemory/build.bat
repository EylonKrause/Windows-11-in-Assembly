@echo off
REM changes/266-rtliszeromemory/build.bat -- assemble, gate on correctness, then benchmark.
setlocal
call "C:\Program Files (x86)\Microsoft Visual Studio\18\BuildTools\VC\Auxiliary\Build\vcvarsall.bat" x64 -vcvars_ver=14.50 >nul 2>&1
set H=%~dp0
cd /d "%H%"
rem  Nothing is linked from another change: this is one VPTEST scan and the only dependency is
rem  the ISA -- AVX2, for VPTEST.
