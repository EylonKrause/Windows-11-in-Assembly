...
extern NTSTATUS wia_oem2un(wchar_t*, ULONG, PULONG, const char*, ULONG);
NTSTATUS ref_oem2un(unsigned short*, unsigned long, unsigned long*, const unsigned char*, unsigned long);
...
static int one(fn sys,const char*src,int n,ULONG mx){
    ...
    NTSTATUS s1=sys(d1,mx,&l1,src,n), s2=wia_oem2un(d2,mx,&l2,src,n);
    NTSTATUS sr=ref_oem2un((void*)dr,mx,&lr,(const unsigned char*)src,n);
...
int main(void){
    wia_oem2umap_init();
    HMODULE h=LoadLibraryW(L"ntdll.dll"); fn sys=(fn)GetProcAddress(h,"RtlOemToUnicodeN");
    ...
    if(!failures) printf("CORRECTNESS: PASS (OEM->UTF-16 N n=0..280, ASCII+highbytes + truncation, vs ntdll)\n");
