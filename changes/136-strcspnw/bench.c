...
#pragma optimize("", off)
static uint64_t op_ours(void*c){ CASE*k=(CASE*)c; return (uint64_t)wia_strcspnw(k->s,k->set); }
...
int main(void){
    ...
    return wia_bench_compare("shlwapi StrCSpnW (wia AVX2 vs shlwapi)", cs, 5, 200);
}
