@echo off
REM changes/181-strupr-s/build.bat -- assemble, gate on correctness, then benchmark.
REM /MD is REQUIRED, not cosmetic: with the default static CRT this exe would carry its OWN
REM invalid-parameter handler state, so ucrtbase's _strupr_s and our _invalid_parameter_noinfo
