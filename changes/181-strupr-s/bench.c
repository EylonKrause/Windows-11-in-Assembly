// changes/181-strupr-s/bench.c
// Gate 2: time wia_strupr_s against the live ucrtbase!_strupr_s across size classes.
...
static uint64_t op_ours(void* c){ CASE* k=(CASE*)c; memcpy(work,k->src,(size_t)(k->n+1));
                                  return (uint64_t)wia_strupr_s(work,k->cch); }
...
int main(void){
    ...
    /* a realistic mix: lowercase, already-uppercase, digits and high bytes that must not fold */
    for(int i=0;i<254;i++){
        int m=i%4;
    ...
    return wia_bench_compare("ucrtbase _strupr_s (wia AVX2 validate-then-fold vs ucrtbase scalar)", cs, N, 300);
}
