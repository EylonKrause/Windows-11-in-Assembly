@echo off
REM changes/185-wcsnset-s/build.bat -- assemble, gate on correctness, then benchmark.
Rem /md is required, not cosmetic: with the default static CRT this exe would carry its own
REM invalid-parameter handler state, so ucrtbase's _wcsnset_s and our _invalid_parameter_noinfo
