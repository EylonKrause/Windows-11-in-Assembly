# 008-rtlcompareunicodestring — TGL variant → **UNPROVEN** (created 2026-09-20)
...
The 8-byte case-insensitive class measures 0.94x here. Eight bytes is four wchars, so the number is almost entirely fixed overhead: the parent pays five push/pop pairs before the length is known, plus a `vzeroupper` the scalar path never earned, and then two DEPENDENT loads per character (load the wchar, then index the OS upcase table with the value just loaded). The variant dispatches the short CI case using only volatile registers and consults the table only when the two RAW wchars differ.
