...
int ref_ultow_s(unsigned long, wchar_t*, size_t, int);
typedef int (__cdecl *FN)(unsigned long, wchar_t*, size_t, int);
static FN sys;
...
static unsigned long sd = 0x20100u;
...
static int one(unsigned long v, size_t size, int radix){
    ...
    long h0, h2;
    *sys_errno()=0; hits=0; int ra = wia_ultow_s(v,a,size,radix); h0=hits; int ea=*sys_errno();
    *sys_errno()=0;         int rb = ref_ultow_s(v,b,size,radix);          int eb=*sys_errno();
...
static int one_null(unsigned long v, size_t size, int radix){
    *sys_errno()=0; hits=0; int ra = wia_ultow_s(v,NULL,size,radix); long h0=hits; int ea=*sys_errno();
...
int main(void){
    ...
    sys = (FN)GetProcAddress(hu,"_ultow_s");
    sys_errno = (int*(__cdecl*)(void))GetProcAddress(hu,"_errno");
    if(!sys||!sys_errno){ printf("CORRECTNESS: cannot resolve ucrtbase!_ultow_s/_errno\n"); return 1; }
    {
    ...
    // ---- page guard: the buffer ends exactly at a PAGE_NOACCESS boundary ----
    {
        ...
        for(size_t s=1; s<=70; ++s){
            for(int r=2;r<=36;r+=7){
                ...
                *sys_errno()=0; hits=0; int ra = wia_ultow_s(3456789012UL,p,s,r); long h0=hits;
    ...
    printf("CORRECTNESS: PASS (_ultow_s vs live ucrtbase + oracle -- return, WHOLE buffer (the "
