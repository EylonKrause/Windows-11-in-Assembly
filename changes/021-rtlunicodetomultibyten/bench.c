...
static uint64_t op_ours(void*c){ ctx_t*m=(ctx_t*)c; ULONG o=0; wia_u2mb(m->dst,m->mx,&o,m->src,m->sb); return o; }
static uint64_t op_sys (void*c){ ctx_t*m=(ctx_t*)c; ULONG o=0; sys(m->dst,m->mx,&o,m->src,m->sb); return o; }
int main(void){
    wia_ansimap_init();
    HMODULE h=LoadLibraryW(L"ntdll.dll"); sys=(fn)GetProcAddress(h,"RtlUnicodeToMultiByteN");
    ...
    return wia_bench_compare("RtlUnicodeToMultiByteN  (wia AVX2 vs ntdll)", cs, K, 200);
}
