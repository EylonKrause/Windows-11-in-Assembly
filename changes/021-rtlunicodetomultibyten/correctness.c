...
extern NTSTATUS wia_u2mb(char*, ULONG, PULONG, const wchar_t*, ULONG);
NTSTATUS ref_u2mb(unsigned char*, unsigned long, unsigned long*, const unsigned short*, unsigned long);
...
static int one(fn sys,const wchar_t*src,int n,ULONG mx){
    ...
    NTSTATUS s1=sys(d1,mx,&l1,src,n*2), s2=wia_u2mb(d2,mx,&l2,src,n*2);
    NTSTATUS sr=ref_u2mb((void*)dr,mx,&lr,(const unsigned short*)src,n*2);
...
int main(void){
    wia_ansimap_init();
    HMODULE h=LoadLibraryW(L"ntdll.dll"); fn sys=(fn)GetProcAddress(h,"RtlUnicodeToMultiByteN");
    ...
    if(!failures) printf("CORRECTNESS: PASS (UTF-16->ANSI N n=0..280, ASCII+nonASCII + overflow, vs ntdll)\n");
