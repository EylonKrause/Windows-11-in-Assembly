// changes/084-cryptstringtobinaryw-base64/bench.c
...
static decf sys;
typedef struct { wchar_t* b64; unsigned long slen; unsigned char* out; } ctx_t;
#pragma optimize("", off)
static uint64_t op_ours(void*c){ ctx_t*m=(ctx_t*)c; unsigned long cb=49152,sk,fl; wia_s2bw(m->b64,m->slen,0x1u,m->out,&cb,&sk,&fl); return cb; }
static uint64_t op_sys (void*c){ ctx_t*m=(ctx_t*)c; unsigned long cb=49152,sk,fl; sys(m->b64,m->slen,0x1u,m->out,&cb,&sk,&fl); return cb; }
#pragma optimize("", on)
int main(void){
    wia_b64rev_init();
    ...
    for(int i=0;i<K;++i){ unsigned char* b=malloc(L[i]); for(int k=0;k<L[i];k++)b[k]=(unsigned char)(k*131+7);
        wchar_t* s=malloc((L[i]*2+64)*2); unsigned long cch=L[i]*2+64; enc(b,L[i],0x1u|0x40000000u,s,&cch);
        cx[i].b64=s; cx[i].slen=cch; cx[i].out=malloc(L[i]+64);
        cs[i].label=N[i]; cs[i].bytes=L[i]; cs[i].ours=op_ours; cs[i].system=op_sys; cs[i].ctx=&cx[i]; }
    return wia_bench_compare("CryptStringToBinaryW base64 decode  (wia SSSE3 vs crypt32)", cs, K, 150);
}
