...
static int one(const char* in, size_t n){
    ...
    a[k]=b[k]=c[k]=0;
    int ra = wia_strlwr_s(a,n);
    int rb = ref_strlwr_s(b,n);
    int rc = sys(c,n);
...
int main(void){
    ...
    if(!sys){ printf("CORRECTNESS: cannot resolve ucrtbase!_strlwr_s\n"); return 1; }
    {
    ...
    {
        static const unsigned char edge[7] = { 0x40,0x41,0x5A,0x5B,0x61,0x80,0xFF };
    ...
    // 16 unaligned start offsets
    {
        ...
        for(int off=0; off<16; ++off){
            ...
            for(int len=0; len<=80; ++len){
                ...
                int ra = wia_strlwr_s(a,(size_t)len+1);
                int rb = ref_strlwr_s(b,(size_t)len+1);
                int ok = (ra==rb);
    ...
    // page guard: string ending exactly at a PAGE_NOACCESS boundary, bound = exactly len+1
    {
        ...
        for(int tail=1; tail<=130; ++tail){
            ...
            int ra = wia_strlwr_s(p,(size_t)tail);       // must not read into page 2
            int rb = ref_strlwr_s(b,(size_t)tail);
    ...
    printf("CORRECTNESS: PASS (_strlwr_s vs live ucrtbase + oracle, return + whole buffer: "
