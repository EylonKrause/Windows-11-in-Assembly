// changes/085-cryptbinarytostring-hexraw/bench.c
...
#pragma optimize("", off)
static uint64_t op_ours(void*c){ ctx_t*m=(ctx_t*)c; unsigned long cch=m->cap; wia_b2sh(m->bin,m->n,0x0cu|0x40000000u,m->out,&cch); return cch; }
...
int main(void){
    ...
    return wia_bench_compare("CryptBinaryToStringA HEXRAW  (wia SSSE3 vs crypt32)", cs, K, 150);
}
