// changes/140-pathremoveextensionw/bench.c
...
#pragma optimize("", off)
static uint64_t op_ours(void*c){ CASE*k=(CASE*)c; memcpy(work,k->src,(size_t)(k->n+1)*2); wia_pathremoveextw(work); return work[0]; }
...
int main(void){
    ...
    return wia_bench_compare("shlwapi PathRemoveExtensionW (wia AVX2 vs shlwapi)", cs, 5, 300);
}
