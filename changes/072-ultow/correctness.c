// changes/072-ultow/correctness.c
...
static void one(fn sys, unsigned long v, int radix){
    ...
    wchar_t* ro=wia_ultow(v,bo,radix); wchar_t* ry=sys(v,by,radix); wchar_t* rr=ref_ultow(v,br,radix);
    int bad=(ro!=bo)||(ry!=by)||(rr!=br)||wcscmp(bo,by)!=0||wcscmp(bo,br)!=0;
    if(bad){ printf("FAIL v=%lu radix=%d: ours='%ls' sys='%ls' ref='%ls'\n",v,radix,bo,by,br); ++failures; }
}
int main(void){
    wia_dec2_init();
    HMODULE h=LoadLibraryW(L"ucrtbase.dll");
    fn sys=(fn)GetProcAddress(h,"_ultow");
    if(!sys){printf("no _ultow\n");return 2;}
    unsigned long seed=0x72abcu;
    ...
    if(!failures) printf("CORRECTNESS: PASS (_ultow radix 2..36 AND every out-of-range radix that returns x values 0..2000 + edges + 8000 random/radix, vs ucrtbase)\n");
