/* changes/031-rtlupcaseunicodetooemn/bench.c
 *
...
static uint64_t op_ours(void*c){ ctx_t*m=(ctx_t*)c; ULONG o=0; wia_u2uoem(m->dst,m->mx,&o,m->src,m->sb); return o; }
...
int main(void){
    ...
    int c, li, r = 0;
    wia_upoemmap_init();
    h = LoadLibraryW(L"ntdll.dll"); sys = (fn_t)GetProcAddress(h, "RtlUpcaseUnicodeToOemN");
    for (c = 0; c < CLASSES; ++c) {
        for (li = 0; li < LENS; ++li) {
            int n = L[li];
    ...
    return wia_bench_compare(
        "RtlUpcaseUnicodeToOemN  (wia AVX2 vs ntdll, every input class -- not just ASCII)", cs, r, 100);
}
