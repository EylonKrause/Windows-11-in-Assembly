// changes/199-ultoa-s/correctness.c
// Gate 1: wia_ultoa_s must be indistinguishable from ucrtbase!_ultoa_s, the return value, the
...
int ref_ultoa_s(unsigned long, char*, size_t, int);
typedef int (__cdecl *FN)(unsigned long, char*, size_t, int);
static FN sys;
...
static unsigned long sd = 0x19900u;
static unsigned rnd(void){ sd = sd*1103515245u + 12345u; return sd>>8; }

static int one(unsigned long v, size_t size, int radix){
    static char a[DSZ], b[DSZ], c[DSZ];
    for(int i=0;i<DSZ;i++){ a[i]=PB; b[i]=PB; c[i]=PB; }
    long h0, h2;
    *sys_errno()=0; hits=0; int ra = wia_ultoa_s(v,a,size,radix); h0=hits; int ea=*sys_errno();
    *sys_errno()=0;         int rb = ref_ultoa_s(v,b,size,radix);          int eb=*sys_errno();
...
static int one_null(unsigned long v, size_t size, int radix){
    *sys_errno()=0; hits=0; int ra = wia_ultoa_s(v,NULL,size,radix); long h0=hits; int ea=*sys_errno();
...
int main(void){
    ...
    sys = (FN)GetProcAddress(hu,"_ultoa_s");
    sys_errno = (int*(__cdecl*)(void))GetProcAddress(hu,"_errno");
    if(!sys||!sys_errno){ printf("CORRECTNESS: cannot resolve ucrtbase!_ultoa_s/_errno\n"); return 1; }
    {
    ...
    // ---- every size 0..48 x several values x every radix: the whole ERANGE partial surface ----
    {
        static const unsigned long V[] = { 0, 1, 9, 10, 99, 100, 1234,
                                       ...
                                       4294967294UL, 65535UL, 65536UL,
                                       1000000000UL, 3000000000UL, 7UL };
    ...
    for(int r=2;r<=36;r++){
        for(int k=0;k<32;k++){
            unsigned long v = (k<31) ? (unsigned long)(1u<<k) : (unsigned long)(k==31 ? 0x80000000u : 0xFFFFFFFFu);
    ...
    for(int t=0;t<300000;++t){
        ...
        unsigned long v = (unsigned long)(unsigned)u;
        if(rnd()%4==0) v = (unsigned long)(unsigned short)rnd();
        int r = 2 + (int)(rnd()%35);
    ...
    // ---- page guard: the buffer ends exactly at a PAGE_NOACCESS boundary ----
    {
        ...
        for(size_t s=1; s<=70; ++s){
            for(int r=2;r<=36;r+=7){
                ...
                *sys_errno()=0; hits=0; int ra = wia_ultoa_s(3456789012UL,p,s,r); long h0=hits;
                for(size_t i=0;i<s;i++) b[i]=p[i];
                for(size_t i=0;i<s;i++) p[i]=PB;
                *sys_errno()=0; hits=0; int rc = sys(3456789012UL,p,s,r); long h2=hits;
                int ok = (ra==rc) && (h0==h2);
    ...
    printf("CORRECTNESS: PASS (_ultoa_s vs live ucrtbase + oracle -- return, WHOLE buffer (the "
