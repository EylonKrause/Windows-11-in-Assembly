// changes/199-ultoa-s/reference.c
// The correctness oracle for ucrtbase!_ultoa_s. Not fast; just correct.
//
...
int ref_ultoa_s_hits = 0;
int ref_ultoa_s_errno = 0;

int ref_ultoa_s(unsigned long value, char* buf, size_t size, int radix){
    ref_ultoa_s_hits = 0; ref_ultoa_s_errno = 0;
    if(buf == 0 || size == 0){ ref_ultoa_s_hits = 1; ref_ultoa_s_errno = 22; return 22; }
    buf[0] = 0;
    int neg = 0;                                 /* the unsigned form never signs */
    unsigned uv = (unsigned)value;
    if(size <= (size_t)neg + 1){ ref_ultoa_s_hits = 1; ref_ultoa_s_errno = 34; return 34; }
    if((unsigned)(radix - 2) > 34u){ ref_ultoa_s_hits = 1; ref_ultoa_s_errno = 22; return 22; }
    ...
    ref_ultoa_s_hits = 1; ref_ultoa_s_errno = 34;
    return 34;
}
