...
static uint64_t op_ours(void*c){ ctx_t*m=(ctx_t*)c; return wia_rtlequstr_a(&m->u1,&m->u2,m->ci); }
...
int main(void){
    ...
    HMODULE h=LoadLibraryW(L"ntdll.dll"); sys=(fn)GetProcAddress(h,"RtlEqualString");
    ...
    return wia_bench_compare("RtlEqualString  (wia AVX2 vs ntdll; cs + CI)", cs, 2*K, 200);
}
