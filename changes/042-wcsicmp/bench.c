...
#pragma optimize("", off)
static uint64_t op_ours(void*c){ ctx_t*m=(ctx_t*)c; return (uint64_t)(unsigned)wia_wcsicmp(m->a,m->b); }
static uint64_t op_sys (void*c){ ctx_t*m=(ctx_t*)c; return (uint64_t)(unsigned)sys(m->a,m->b); }
#pragma optimize("", on)
int main(void){
    HMODULE h=LoadLibraryW(L"ucrtbase.dll"); sys=(fn)GetProcAddress(h,"_wcsicmp");
    ...
    for(int i=0;i<K;++i){
        ...
        for(int k=0;k<L[i];k++){ a[k]=(wchar_t)(L'A'+(k%23)); b[k]=(wchar_t)(L'a'+(k%23)); } // case-equal -> full scan
        a[L[i]]=b[L[i]]=0;
        cx[i].a=a; cx[i].b=b;
        cs[i].label=N[i]; cs[i].bytes=L[i]*2; cs[i].ours=op_ours; cs[i].system=op_sys; cs[i].ctx=&cx[i];
    }
    return wia_bench_compare("_wcsicmp  (wia AVX2 in-reg ASCII fold vs ucrtbase, case-equal, full scan)", cs, K, 200);
}
