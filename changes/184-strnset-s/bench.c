// changes/184-strnset-s/bench.c
// Gate 2: time wia_strnset_s against the live ucrtbase!_strnset_s across size classes.
...
int main(void){
    ...
    static const size_t bytes[] = { 8,32,64,254,2048,64,127 };
    for(int i=0;i<N;i++){ cs[i].label=names[i]; cs[i].bytes=bytes[i];
        cs[i].ours=op_ours; cs[i].system=op_sys; cs[i].ctx=&C[i]; }
    return wia_bench_compare("ucrtbase _strnset_s (wia AVX2 scan + clipped broadcast fill vs ucrtbase scalar)", cs, N, 300);
}
