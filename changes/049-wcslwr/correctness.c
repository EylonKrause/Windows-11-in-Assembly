...
unsigned short* ref_wcslwr(unsigned short*);
...
int main(void){
    ...
    fn sys=(fn)GetProcAddress(h,"_wcslwr");
    if(!sys){printf("no _wcslwr\n");return 2;}
    ...
    for(size_t len=0; len<=320; ++len){
        for(int off=0; off<8; ++off){
            ...
            wchar_t* ro=wia_wcslwr(O+off);
            wchar_t* ry=sys(Y+off);
            unsigned short* rr=ref_wcslwr((unsigned short*)(R+off));
...
endt:;
    ...
    for(int tail=2; tail<=200; tail+=2){
        ...
        for(wchar_t* p=start;p<term;++p){ unsigned long s2=(unsigned long)(size_t)p; *p=(wchar_t)(0x41+((s2>>3)%40)); }
        *term=0;
        static wchar_t rbuf[300]; int n=(int)(term-start);
        memcpy(rbuf,start,(n+1)*2);
        wia_wcslwr(start);
        ref_wcslwr((unsigned short*)rbuf);
        if(memcmp(start,rbuf,(n+1)*2)!=0){ printf("FAIL pg tail=%d\n",tail); if(++failures>8) break; }
    }
    if(!failures) printf("CORRECTNESS: PASS (_wcslwr fuzz 0..320 x8 align: whole-buffer match (no over-write) + non-ascii + page-guard, vs ucrtbase)\n");
