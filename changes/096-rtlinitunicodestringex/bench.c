...
#pragma optimize("", off)
static uint64_t op_ours(void*c){ ctx_t*m=(ctx_t*)c; wia_rtlinitusex(&m->us,m->s); return m->us.Length; }
static uint64_t op_sys (void*c){ ctx_t*m=(ctx_t*)c; sys(&m->us,m->s); return m->us.Length; }
#pragma optimize("", on)
int main(void){
    HMODULE h=GetModuleHandleW(L"ntdll.dll"); sys=(fn)GetProcAddress(h,"RtlInitUnicodeStringEx");
    ...
    return wia_bench_compare("RtlInitUnicodeStringEx (wia inline AVX2 wcslen vs ntdll call+wcslen)", cs, K, 200);
}
