// changes/197-ui64tow-s/correctness.c
// Gate 1: wia_ui64tow_s must be indistinguishable from ucrtbase!_ui64tow_s -- the return value, the
...
int ref_ui64tow_s(unsigned long long, wchar_t*, size_t, int);
...
static unsigned long sd = 0x19700u;
...
static int one(unsigned long long v, size_t size, int radix){
    ...
    long h0, h2;
    *sys_errno()=0; hits=0; int ra = wia_ui64tow_s(v,a,size,radix); h0=hits; int ea=*sys_errno();
    *sys_errno()=0;         int rb = ref_ui64tow_s(v,b,size,radix);          int eb=*sys_errno();
...
static int one_null(unsigned long long v, size_t size, int radix){
    *sys_errno()=0; hits=0; int ra = wia_ui64tow_s(v,NULL,size,radix); long h0=hits; int ea=*sys_errno();
...
int main(void){
    ...
    if(!sys||!sys_errno){ printf("CORRECTNESS: cannot resolve ucrtbase!_ui64tow_s/_errno\n"); return 1; }
    {
    ...
    // ---- page guard: the buffer ends exactly at a PAGE_NOACCESS boundary ----
    {
        ...
        for(size_t s=1; s<=70; ++s){
            for(int r=2;r<=36;r+=7){
                wchar_t* p = (wchar_t*)((base+pg) - s*2);
                for(size_t i=0;i<s;i++) p[i]=PB;
                for(int i=0;i<DSZ;i++) b[i]=PB;
                *sys_errno()=0; hits=0; int ra = wia_ui64tow_s(12345678901234567890ULL,p,s,r); long h0=hits;
    ...
    printf("CORRECTNESS: PASS (_ui64tow_s vs live ucrtbase + oracle -- return, WHOLE buffer (the "
