// changes/141-pathremoveblanksw/bench.c
...
#pragma optimize("", off)
static uint64_t op_ours(void*c){ CASE*k=(CASE*)c; memcpy(work,k->src,(size_t)(k->n+1)*2); wia_pathremoveblanksw(work); return work[0]; }
...
static void fill(wchar_t* b,int n){ for(int i=0;i<n;i++) b[i]=L'a'+(i%23); if(n>2){b[0]=L' ';b[1]=L' ';b[n-1]=L' ';} b[n]=0; }
int main(void){
    ...
    return wia_bench_compare("shlwapi PathRemoveBlanksW (wia AVX2 vs shlwapi)", cs, 5, 300);
}
