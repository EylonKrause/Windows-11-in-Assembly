...
#pragma optimize("", off)
static uint64_t op_ours(void* c){ CASE* k=(CASE*)c; return (uint64_t)wia_ui64toa_s(k->v,obuf,k->size,k->radix); }
...
int main(void){
    ...
    return wia_bench_compare("ucrtbase _ui64toa_s (wia table/shift digits vs ucrtbase div-per-digit)", cs, N, 300);
}
