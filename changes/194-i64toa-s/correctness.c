...
int ref_i64toa_s(long long, char*, size_t, int);
...
static unsigned long sd = 0x19400u;
...
static int one(long long v, size_t size, int radix){
    ...
    long h0, h2;
    *sys_errno()=0; hits=0; int ra = wia_i64toa_s(v,a,size,radix); h0=hits; int ea=*sys_errno();
    *sys_errno()=0;         int rb = ref_i64toa_s(v,b,size,radix);          int eb=*sys_errno();
...
static int one_null(long long v, size_t size, int radix){
    *sys_errno()=0; hits=0; int ra = wia_i64toa_s(v,NULL,size,radix); long h0=hits; int ea=*sys_errno();
...
int main(void){
    ...
    sys = (FN)GetProcAddress(hu,"_i64toa_s");
    sys_errno = (int*(__cdecl*)(void))GetProcAddress(hu,"_errno");
    if(!sys||!sys_errno){ printf("CORRECTNESS: cannot resolve ucrtbase!_i64toa_s/_errno\n"); return 1; }
    {
    ...
    // ---- page guard: the buffer ends exactly at a PAGE_NOACCESS boundary ----
    {
        ...
        for(size_t s=1; s<=70; ++s){
            for(int r=2;r<=36;r+=7){
                ...
                *sys_errno()=0; hits=0; int ra = wia_i64toa_s(-1234567890123LL,p,s,r); long h0=hits;
    ...
    printf("CORRECTNESS: PASS (_i64toa_s vs live ucrtbase + oracle -- return, WHOLE buffer (the "
