// changes/214-strcspna/correctness.c
// Gate 1: wia_strcspna must be indistinguishable from shlwapi!StrCSpnA.
//
...
int ref_strcspna(const char*, const char*);
...
static int one(const char* s, const char* set){
    int a = wia_strcspna(s, set);
    int b = ref_strcspna(s, set);
    int c = sys(s, set);
    return a == b && a == c;
}
...
int main(void){
    ...
    sys = (FN)GetProcAddress(hs, "StrCSpnA");
    if (!sys) { printf("CORRECTNESS: cannot resolve StrCSpnA\n"); return 1; }

    // ---- NULL, and the NULL-is-not-empty distinction ----
    CHECK(wia_strcspna(NULL, "abc") == sys(NULL, "abc"), "NULL subject");
    CHECK(wia_strcspna("abc", NULL) == sys("abc", NULL), "NULL set");
    CHECK(wia_strcspna(NULL, NULL)  == sys(NULL, NULL),  "both NULL");
    CHECK(wia_strcspna("abc", NULL) == 0, "a NULL set returns 0");
    ...
    printf("CORRECTNESS: PASS (StrCSpnA vs live shlwapi + oracle: NULL arguments, including the "
           "distinction a reimplementation is most likely to miss -- a NULL set returns 0 while an "
           "EMPTY set returns strlen; EVERY byte value 0x01..0xFF proved both as a set member and as "
