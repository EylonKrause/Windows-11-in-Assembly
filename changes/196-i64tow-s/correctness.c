// changes/196-i64tow-s/correctness.c
// Gate 1: wia_i64tow_s must be indistinguishable from ucrtbase!_i64tow_s -- the return value, the
...
int ref_i64tow_s(long long, wchar_t*, size_t, int);
typedef int (__cdecl *FN)(long long, wchar_t*, size_t, int);
static FN sys;
...
static unsigned long sd = 0x19600u;
static unsigned rnd(void){ sd = sd*1103515245u + 12345u; return sd>>8; }

static int one(long long v, size_t size, int radix){
    static wchar_t a[DSZ], b[DSZ], c[DSZ];
    for(int i=0;i<DSZ;i++){ a[i]=PB; b[i]=PB; c[i]=PB; }
    long h0, h2;
    *sys_errno()=0; hits=0; int ra = wia_i64tow_s(v,a,size,radix); h0=hits; int ea=*sys_errno();
    *sys_errno()=0;         int rb = ref_i64tow_s(v,b,size,radix);          int eb=*sys_errno();
...
static int one_null(long long v, size_t size, int radix){
    *sys_errno()=0; hits=0; int ra = wia_i64tow_s(v,NULL,size,radix); long h0=hits; int ea=*sys_errno();
...
int main(void){
    ...
    sys = (FN)GetProcAddress(hu,"_i64tow_s");
    sys_errno = (int*(__cdecl*)(void))GetProcAddress(hu,"_errno");
    if(!sys||!sys_errno){ printf("CORRECTNESS: cannot resolve ucrtbase!_i64tow_s/_errno\n"); return 1; }
    {
    ...
    // ---- every size 0..48 x several values x every radix: the whole ERANGE partial surface ----
    {
        static const long long V[] = { 0, 1, 9, 10, 99, 100, 1234, -1, -9, -10, -1234,
                                       ...
                                       4294967295LL, -4294967296LL, 1000000000000000000LL };
    ...
    // ---- page guard: the buffer ends exactly at a PAGE_NOACCESS boundary ----
    {
        ...
        for(size_t s=1; s<=70; ++s){
            for(int r=2;r<=36;r+=7){
                ...
                *sys_errno()=0; hits=0; int ra = wia_i64tow_s(-1234567890123LL,p,s,r); long h0=hits;
                for(size_t i=0;i<s;i++) b[i]=p[i];
                for(size_t i=0;i<s;i++) p[i]=PB;
                *sys_errno()=0; hits=0; int rc = sys(-1234567890123LL,p,s,r); long h2=hits;
                int ok = (ra==rc) && (h0==h2);
    ...
    printf("CORRECTNESS: PASS (_i64tow_s vs live ucrtbase + oracle -- return, WHOLE buffer (the "
