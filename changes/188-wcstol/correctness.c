// changes/188-wcstol/correctness.c
// Gate 1: wia_wcstol must be indistinguishable from ucrtbase!wcstol -- value, *endptr, errno AND
...
long ref_wcstol(const unsigned short*, unsigned short**, int);
int  ref_dv36(unsigned);
extern int ref_wcstol_badbase;
...
static unsigned long sd = 0x18800u;
...
static int one(const wchar_t* s, int base){
    ...
    *sys_errno()=0; hits=0; long va = wia_wcstol(s,&ea,base);           h_a = hits; int ra = *sys_errno();
    *sys_errno()=0;         long vb = ref_wcstol((const unsigned short*)s,&eb,base); int rb = *sys_errno();
    *sys_errno()=0; hits=0; long vc = sys(s,&ec,base);                  h_c = hits; int rc = *sys_errno();
...
static int one_nullep(const wchar_t* s, int base){
    *sys_errno()=0; hits=0; long va = wia_wcstol(s,NULL,base); long h_a=hits; int ra=*sys_errno();
    *sys_errno()=0; hits=0; long vc = sys(s,NULL,base);        long h_c=hits; int rc=*sys_errno();
...
int main(void){
    ...
    if(!sys||!sys_errno){ printf("CORRECTNESS: cannot resolve ucrtbase!wcstol/_errno\n"); return 1; }
    {
    ...
    // ---- page guard ----
    {
        ...
        for(int tail=1; tail<=40; ++tail){
            for(int k=0;k<NB;k++){
                ...
                CHECK(wia_wcstol(p,&e1,BASES[k])==sys(p,&e2,BASES[k]) && e1==e2,
            ...
            p[0]=DBLK[3]; p[1]=0;
            wchar_t *e1=SENT,*e2=SENT;
            CHECK(wia_wcstol(p,&e1,16)==sys(p,&e2,16) && e1==e2, "page-guard: prefix probe, base 16");
            CHECK(wia_wcstol(p,&e1,0) ==sys(p,&e2,0)  && e1==e2, "page-guard: prefix probe, base 0");
        }
        VirtualFree(base,0,MEM_RELEASE);
    }

    if(fails){ printf("CORRECTNESS: FAILED (%d)\n", fails); return 1; }
    printf("CORRECTNESS: PASS (wcstol vs live ucrtbase + oracle -- value, *endptr, errno AND "
           ...
           "non-ASCII block, 1.5M fuzz, NOACCESS page-guard incl. the prefix probe at the edge)\n");
    return 0;
}
