@echo off
REM changes/230-lstrcatw/build.bat -- assemble, gate on correctness, then benchmark.
REM /EHa is required: the wrapper in seh.c converts an access violation into NULL, which is the
REM measured behaviour of the shipped export (see probes/catw.c).
setlocal
...
cl /nologo /O2 /EHa /c seh.c /Fo:seh230.obj >nul || goto :err
cl /nologo /O2 /EHa correctness.c reference.c impl.obj seh230.obj /Fe:correctness.exe >nul || goto :err
"%H%correctness.exe" || (echo CORRECTNESS FAILED - not benchmarking & goto :err)
cl /nologo /O2 /I "%H%..\..\harness" bench.c impl.obj seh230.obj /Fe:bench.exe >nul || goto :err
"%H%bench.exe"
