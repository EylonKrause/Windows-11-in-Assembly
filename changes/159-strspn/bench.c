...
#pragma optimize("", off)
static uint64_t op_ours(void* x){ CASE* k=(CASE*)x; return (uint64_t)wia_strspn(k->s,k->set); }
...
int main(void){
    HMODULE u=LoadLibraryW(L"ucrtbase.dll"); sys=(fn)GetProcAddress(u,"strspn");
    fill(s16,16); fill(s64,64); fill(s254,254); fill(s1024,1024);
    memset(searly,'a',254); searly[254]=0; searly[8]='Z';   /* the span stops at 8 */
    ...
    C[0].set=C[1].set=C[2].set=C[3].set="abc";
    C[4].set="a";
    C[5].set="abcdefghijklmnop";
    for(int i=0;i<6;++i){ cs[i].label=N[i]; cs[i].bytes=L[i]; cs[i].ours=op_ours; cs[i].system=op_sys; cs[i].ctx=&C[i]; }
    return wia_bench_compare("ucrtbase strspn (wia AVX2 vs ucrtbase)", cs, 6, 300);
}
