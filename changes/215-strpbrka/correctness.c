// changes/215-strpbrka/correctness.c
// Gate 1: wia_strpbrka must be indistinguishable from shlwapi!StrPBrkA.
//
...
const char* ref_strpbrka(const char*, const char*);
...
/* compared as offsets-or-absent, which is what a caller can observe */
static int one(const char* s, const char* set){
    const char* a = wia_strpbrka(s, set);
    const char* b = ref_strpbrka(s, set);
    char* c = sys(s, set);
...
int main(void){
    ...
    sys = (FN)GetProcAddress(hs, "StrPBrkA");
    if (!sys) { printf("CORRECTNESS: cannot resolve StrPBrkA\n"); return 1; }

    // ---- NULL and the empty set. For THIS export the two agree (both NULL), unlike its sibling
    // ---- StrCSpnA where a NULL set gives 0 and an EMPTY set gives strlen. Measured, not assumed,
    // ---- because the two functions share a core and the wrong rule would carry across silently.
    CHECK((wia_strpbrka(NULL, "abc") == NULL) == (sys(NULL, "abc") == NULL), "NULL subject");
    CHECK((wia_strpbrka("abc", NULL) == NULL) == (sys("abc", NULL) == NULL), "NULL set");
    CHECK((wia_strpbrka(NULL, NULL)  == NULL) == (sys(NULL, NULL)  == NULL), "both NULL");
    CHECK(wia_strpbrka("abc", NULL) == NULL, "a NULL set returns NULL");
    CHECK(one("abc", ""), "an EMPTY set agrees with the live export");
    CHECK(wia_strpbrka("abc", "") == NULL, "and that value is NULL");
    ...
    printf("CORRECTNESS: PASS (StrPBrkA vs live shlwapi + oracle: NULL arguments, including the "
           "distinction a reimplementation is most likely to miss -- here a NULL set and an EMPTY "
           "set BOTH return NULL, where the sibling StrCSpnA sharing this core gives 0 and strlen "
           "respectively, so the rule is measured rather than carried across; EVERY byte value 0x01..0xFF proved both as a set member and as "
