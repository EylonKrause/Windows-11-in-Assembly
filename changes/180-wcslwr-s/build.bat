@echo off
REM changes/180-wcslwr-s/build.bat -- assemble, gate on correctness, then benchmark.
REM /MD is REQUIRED, not cosmetic: with the default static CRT this exe would carry its OWN
REM invalid-parameter handler state, so ucrtbase's _wcslwr_s and our _invalid_parameter_noinfo
