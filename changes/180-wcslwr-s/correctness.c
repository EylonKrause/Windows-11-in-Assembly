// changes/180-wcslwr-s/correctness.c
// Gate 1: wia_wcslwr_s must be indistinguishable from ucrtbase!_wcslwr_s.
...
int ref_wcslwr_s(wchar_t*, size_t);
...
static int one(const wchar_t* in, size_t n){
    ...
    a[k]=b[k]=c[k]=0;
    int ra = wia_wcslwr_s(a,n);
    int rb = ref_wcslwr_s(b,n);
    int rc = sys(c,n);
...
int main(void){
    ...
    if(!sys){ printf("CORRECTNESS: cannot resolve ucrtbase!_wcslwr_s\n"); return 1; }
    {
    ...
    {
        static const wchar_t edge[6] = { 0x0040, 0x0041, 0x005A, 0x005B, 0x0061, 0x007A };
    ...
    // 16 unaligned start offsets
    {
        ...
        for(int off=0; off<16; ++off){
            ...
            for(int len=0; len<=64; ++len){
                ...
                int ra = wia_wcslwr_s(a,(size_t)len+1);
                int rb = ref_wcslwr_s(b,(size_t)len+1);
                int ok = (ra==rb);
    ...
    // page guard: string ending exactly at a PAGE_NOACCESS boundary, bound = exactly len+1
    {
        ...
        for(int tail=1; tail<=90; ++tail){
            ...
            int ra = wia_wcslwr_s(p,(size_t)tail);       // must not read into page 2
            int rb = ref_wcslwr_s(b,(size_t)tail);
    ...
    printf("CORRECTNESS: PASS (_wcslwr_s vs live ucrtbase + oracle, return + whole buffer: "
