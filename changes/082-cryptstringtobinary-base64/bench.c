// changes/082-cryptstringtobinary-base64/bench.c
...
static decf sys;
typedef struct { char* b64; unsigned long slen; unsigned char* out; } ctx_t;
#pragma optimize("", off)
static uint64_t op_ours(void*c){ ctx_t*m=(ctx_t*)c; unsigned long cb=49152,sk,fl; wia_s2b(m->b64,m->slen,0x1u,m->out,&cb,&sk,&fl); return cb; }
...
int main(void){
    ...
    for(int i=0;i<K;++i){ unsigned char* b=malloc(L[i]); for(int k=0;k<L[i];k++)b[k]=(unsigned char)(k*131+7);
        char* s=malloc(L[i]*2+64); unsigned long cch=L[i]*2+64; enc(b,L[i],0x1u|0x40000000u,s,&cch);
    ...
    return wia_bench_compare("CryptStringToBinaryA base64 decode  (wia SSSE3 vs crypt32)", cs, K, 150);
}
