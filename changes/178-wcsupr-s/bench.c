// changes/178-wcsupr-s/bench.c
// Gate 2: time wia_wcsupr_s against the live ucrtbase!_wcsupr_s across size classes.
...
static uint64_t op_ours(void* c){ CASE* k=(CASE*)c; memcpy(work,k->src,(size_t)(k->n+1)*2);
                                  return (uint64_t)wia_wcsupr_s(work,k->cch); }
...
int main(void){
    ...
    return wia_bench_compare("ucrtbase _wcsupr_s (wia AVX2 validate-then-fold vs ucrtbase scalar)", cs, N, 300);
}
