...
static uint64_t op_ours(void*c){ ctx_t*m=(ctx_t*)c; m->dst.Length=0; return (uint64_t)(unsigned)wia_upcasestr(&m->dst,&m->src,0); }
static uint64_t op_sys (void*c){ ctx_t*m=(ctx_t*)c; m->dst.Length=0; return (uint64_t)(unsigned)sys(&m->dst,&m->src,FALSE); }
int main(void){
    wia_upcase_init();
    HMODULE h=LoadLibraryW(L"ntdll.dll"); sys=(fn)GetProcAddress(h,"RtlUpcaseUnicodeString");
    ...
    for(int i=0;i<K;++i){ wchar_t*s=malloc((L[i]+1)*2),*d=malloc((L[i]+1)*2); for(int k=0;k<L[i];k++)s[k]=L'a'+(k&15);
    ...
    return wia_bench_compare("RtlUpcaseUnicodeString  (wia AVX2 vs ntdll, no-alloc)", cs, K, 200);
}
