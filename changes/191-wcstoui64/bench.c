// changes/191-wcstoui64/bench.c
// Gate 2: time wia_wcstoui64 against the live ucrtbase!_wcstoui64 across input shapes and bases.
...
static uint64_t op_ours(void* c){ CASE* k=(CASE*)c; wchar_t* e;
                                  return (uint64_t)wia_wcstoui64(k->s,&e,k->base) ^ (uint64_t)(uintptr_t)e; }
...
int main(void){
    ...
    return wia_bench_compare("ucrtbase _wcstoui64 (wia scalar + AVX2 block classifier vs ucrtbase)", cs, N, 300);
}
