// changes/191-wcstoui64/correctness.c
// Gate 1: wia_wcstoui64 must be indistinguishable from ucrtbase!_wcstoui64 -- value, *endptr, errno AND
...
unsigned __int64 ref_wcstoui64(const unsigned short*, unsigned short**, int);

typedef unsigned __int64 (__cdecl *WL)(const wchar_t*, wchar_t**, int);
static WL sys;
...
static unsigned long sd = 0x19100u;
...
static int one(const wchar_t* s, int base){
    ...
    *sys_errno()=0; hits=0; unsigned __int64 va = wia_wcstoui64(s,&ea,base);           h_a = hits; int ra = *sys_errno();
    *sys_errno()=0;         unsigned __int64 vb = ref_wcstoui64((const unsigned short*)s,&eb,base); int rb = *sys_errno();
    *sys_errno()=0; hits=0; unsigned __int64 vc = sys(s,&ec,base);                  h_c = hits; int rc = *sys_errno();
...
static int one_nullep(const wchar_t* s, int base){
    *sys_errno()=0; hits=0; unsigned __int64 va = wia_wcstoui64(s,NULL,base); long h_a=hits; int ra=*sys_errno();
    *sys_errno()=0; hits=0; unsigned __int64 vc = sys(s,NULL,base);        long h_c=hits; int rc=*sys_errno();
...
int main(void){
    ...
    sys = (WL)GetProcAddress(hu,"_wcstoui64");
    sys_errno = (int*(__cdecl*)(void))GetProcAddress(hu,"_errno");
    if(!sys||!sys_errno){ printf("CORRECTNESS: cannot resolve ucrtbase!_wcstoui64/_errno\n"); return 1; }
    {
    ...
    // ---- saturation / ERANGE / no-conversion literals ----
    {
        static const wchar_t* lit[] = {
            ...
            L"-1", L"-2", L"-18446744073709551615", L"-18446744073709551616",
            L"0xfffffffffffffffe", L"0xffffffffffffffff", L"0x10000000000000000", L"-0x1",
    ...
    // ---- page guard ----
    {
        ...
        for(int tail=1; tail<=40; ++tail){
            for(int k=0;k<NB;k++){
                ...
                CHECK(wia_wcstoui64(p,&e1,BASES[k])==sys(p,&e2,BASES[k]) && e1==e2,
            ...
            p[0]=DBLK[3]; p[1]=0;
            wchar_t *e1=SENT,*e2=SENT;
            CHECK(wia_wcstoui64(p,&e1,16)==sys(p,&e2,16) && e1==e2, "page-guard: prefix probe, base 16");
            CHECK(wia_wcstoui64(p,&e1,0) ==sys(p,&e2,0)  && e1==e2, "page-guard: prefix probe, base 0");
        }
        VirtualFree(base,0,MEM_RELEASE);
    }

    if(fails){ printf("CORRECTNESS: FAILED (%d)\n", fails); return 1; }
    printf("CORRECTNESS: PASS (_wcstoui64 vs live ucrtbase + oracle -- value, *endptr, errno AND "
           ...
           "non-ASCII block, the modulo-2^64 negation cases, 1.5M fuzz, NOACCESS page-guard incl. the prefix probe at the edge)\n");
    return 0;
}
