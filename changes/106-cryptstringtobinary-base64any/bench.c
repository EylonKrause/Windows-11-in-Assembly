// changes/106-cryptstringtobinary-base64any/bench.c
...
static fn sys;
typedef struct { const char* s; BYTE* out; } ctx_t;
#pragma optimize("", off)
static uint64_t op_ours(void*c){ ctx_t*m=(ctx_t*)c; DWORD cb=1u<<30,sk=0,ff=0; wia_s2b_any(m->s,0,0x6,m->out,&cb,&sk,&ff); return cb; }
static uint64_t op_sys (void*c){ ctx_t*m=(ctx_t*)c; DWORD cb=1u<<30,sk=0,ff=0; sys(m->s,0,0x6,m->out,&cb,&sk,&ff); return cb; }
#pragma optimize("", on)
int main(void){
    wia_b64rev_init(); HMODULE h=LoadLibraryW(L"crypt32.dll"); sys=(fn)GetProcAddress(h,"CryptStringToBinaryA");
    ...
    for(int i=0;i<K;++i){
        ...
        DWORD c=0; CryptBinaryToStringA(d,n,0x1,NULL,&c); char* b=(char*)malloc(c); DWORD c2=c;
        CryptBinaryToStringA(d,n,0x1,b,&c2);
        cx[i].s=b; cx[i].out=(BYTE*)malloc(n+16);
        cs[i].label=N[i]; cs[i].bytes=n; cs[i].ours=op_ours; cs[i].system=op_sys; cs[i].ctx=&cx[i];
    }
    return wia_bench_compare("CryptStringToBinaryA BASE64_ANY decode (plain base64, wia scalar vs crypt32)", cs, K, 200);
}
