// changes/091-cryptbinarytostringw-hexfmt/bench.c
...
#pragma optimize("", off)
static uint64_t op_ours(void*c){ ctx_t*m=(ctx_t*)c; DWORD cch=1u<<30; wia_b2shfw(m->in,m->n,m->fl,m->out,&cch); return cch; }
...
int main(void){
    ...
    return wia_bench_compare("CryptBinaryToStringW HEXASCIIADDR (wia SSSE3 + widen vs crypt32 scalar)", cs, K, 200);
}
