// changes/157-wcsncat-s/correctness.c
// Bit-exact fuzz of wia_wcsncat_s vs live ucrtbase!wcsncat_s + oracle. Each trial compares the errno
...
extern int wia_wcsncat_s(wchar_t* dst, size_t size, const wchar_t* src, size_t count);
int ref_wcsncat_s(wchar_t* dst, size_t size, const wchar_t* src, size_t count, int* iph);
...
static void seed(int doff, int prelen)
{
    ...
    if (prelen < 0) {
        for (int i = doff; i < DSZ; ++i) dsys[i] = dour[i] = dref[i] = L'Z';
    } else {
        /* a zero-LOW-byte family in dst: a byte-granular walk would find a false terminator */
        for (int i = 0; i < prelen; ++i) dsys[doff+i] = dour[doff+i] = dref[doff+i] = (wchar_t)(0x4100 + (i % 26));
...
static void trial(int soff, int doff, int prelen, int srclen, size_t size, size_t count, const char* what)
{
    ...
    /* alternate zero-high-byte and zero-low-byte families in src for the same reason */
    for (int i = 0; i < srclen; ++i) s[i] = (wchar_t)(((srclen & 1) ? 0x4100 : 0x0041) + (i % 23));
    s[srclen] = 0;
    seed(doff, prelen);
    int hs, ho, hr = 0, rs, ro, rr;
    hits = 0; rs = sys(dsys + doff, size, s, count);            hs = hits;
    hits = 0; ro = wia_wcsncat_s(dour + doff, size, s, count);  ho = hits;
    rr = ref_wcsncat_s(dref + doff, size, s, count, &hr);
...
int main(void)
{
    ...
    sys = (fn)GetProcAddress(u, "wcsncat_s");
    pSetIPH set = (pSetIPH)GetProcAddress(u, "_set_invalid_parameter_handler");
    if (!sys || !set) { printf("no wcsncat_s / handler setter\n"); return 2; }
    set(myiph);
    ...
    /* ---- the argument-validation paths ------------------------------------------------------- */
    {
        ...
        {
            seed(0, 2);
            wchar_t* ds = A[i].nulldst ? 0 : dsys;
            wchar_t* du = A[i].nulldst ? 0 : dour;
            wchar_t* dr = A[i].nulldst ? 0 : dref;
            const wchar_t* s = A[i].nullsrc ? 0 : L"xyz";
            int h1, h2, h3 = 0, r1, r2, r3;
            hits = 0; r1 = sys(ds, A[i].size, s, A[i].count);            h1 = hits;
            hits = 0; r2 = wia_wcsncat_s(du, A[i].size, s, A[i].count);  h2 = hits;
            r3 = ref_wcsncat_s(dr, A[i].size, s, A[i].count, &h3);
    ...
    /* ---- src ending at a page boundary -------------------------------------------------------- */
    {
        ...
        for (int len = 0; len < 150 && fails < 15; ++len)
        {
            ...
            for (int a = 0; a < 3; ++a) for (int b = 0; b < 3; ++b)
            {
                ...
                hits = 0; ro = wia_wcsncat_s(dour, sizes[a], s, counts[b]);  ho = hits;
                rr = ref_wcsncat_s(dref, sizes[a], s, counts[b], &hr);
                if (rs != ro || rs != rr || hs != ho || hs != hr
                    || memcmp(dsys, dour, sizeof dsys) || memcmp(dsys, dref, sizeof dsys))
    ...
    if (!fails)
        printf("CORRECTNESS: PASS (wcsncat_s vs live + oracle: return code, handler-invocation count and every\n"
               "  destination byte, over 16 src alignments x 8 dst alignments x dst prefix 0..40 x src length\n"
