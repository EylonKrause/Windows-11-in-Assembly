...
extern unsigned short wia_downcase[65536];
long ref_downcasestr(USTR* dst, const USTR* src, int alloc){
    ...
    for(int i=0;i<n;i++) dst->Buffer[i]=wia_downcase[src->Buffer[i]];
    return 0;
}
