...
long ref_downcasestr(USTR*, const USTR*, int);
void wia_downcase_init(void);
typedef NTSTATUS (WINAPI *fn)(USTR*,const USTR*,BOOLEAN);
static int failures=0;
int main(void){
    wia_downcase_init();
    HMODULE h=LoadLibraryW(L"ntdll.dll"); fn sys=(fn)GetProcAddress(h,"RtlDowncaseUnicodeString");
    ...
    for(int n=0;n<=280;++n){
        ...
        long yo=sys(&u1,&us,FALSE), oo=wia_downcasestr(&u2,&us,0), ro=ref_downcasestr(&ur,&us,0);
    ...
    { USTR us={40,40,src}; USTR u2={0,20,d2}; if(wia_downcasestr(&u2,&us,0)!=(long)0x80000005){printf("FAIL overflow\n");++failures;} }
    if(!failures) printf("CORRECTNESS: PASS (downcase transform n=0..280, ASCII+nonASCII + overflow, vs ntdll)\n");
