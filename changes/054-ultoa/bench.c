...
static fn sys;
typedef struct { unsigned long v; int radix; char buf[48]; } ctx_t;
#pragma optimize("", off)
static uint64_t op_ours(void*c){ ctx_t*m=(ctx_t*)c; return (uint64_t)(unsigned char)wia_ultoa(m->v,m->buf,m->radix)[0]; }
static uint64_t op_sys (void*c){ ctx_t*m=(ctx_t*)c; return (uint64_t)(unsigned char)sys(m->v,m->buf,m->radix)[0]; }
#pragma optimize("", on)
int main(void){
    wia_dec2b_init();
    HMODULE h=LoadLibraryW(L"ucrtbase.dll"); sys=(fn)GetProcAddress(h,"_ultoa");
    ...
    return wia_bench_compare("_ultoa  (wia 2-digit-table / nibble vs ucrtbase)", cs, K, 300);
}
