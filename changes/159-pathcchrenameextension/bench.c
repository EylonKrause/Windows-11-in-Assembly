// changes/159-pathcchrenameextension/bench.c
...
static uint64_t op_ours(void* x){ CASE* k=(CASE*)x; memcpy(k->d,k->seed,(k->len+1)*2);
    return (uint64_t)(unsigned)wia_pathcchrenameext(k->d,k->cch,k->ext); }
...
int main(void){
    HMODULE kb=LoadLibraryW(L"kernelbase.dll"); sys=(fn)GetProcAddress(kb,"PathCchRenameExtension");
    ...
    return wia_bench_compare("kernelbase PathCchRenameExtension (wia AVX2 vs kernelbase)", cs, 6, 300);
}
