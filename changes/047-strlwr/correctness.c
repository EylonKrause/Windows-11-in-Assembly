...
int main(void){
    ...
    fn sys=(fn)GetProcAddress(h,"_strlwr");
    if(!sys){printf("no _strlwr\n");return 2;}
    ...
    for(size_t len=0; len<=320; ++len){
        for(int off=0; off<8; ++off){
            ...
            char* ro=wia_strlwr((char*)(O+off));
            char* ry=sys((char*)(Y+off));
            char* rr=ref_strlwr((char*)(R+off));
...
endt:;
    ...
    for(int tail=1; tail<=140; tail++){
        ...
        *term=0;
        ...
        wia_strlwr(start);
        ref_strlwr((char*)rbuf);
        if(memcmp(start,rbuf,n+1)!=0){ printf("FAIL pg tail=%d\n",tail); if(++failures>8) break; }
    }
    if(!failures) printf("CORRECTNESS: PASS (_strlwr fuzz 0..320 x8 align: whole-buffer match (no over-write) + page-guard, vs ucrtbase)\n");
