...
unsigned long ref_wcstoul(const unsigned short*, unsigned short**, int);
int  ref_dv36(unsigned);
extern int ref_wcstoul_badbase;

typedef unsigned long (__cdecl *WL)(const wchar_t*, wchar_t**, int);
static WL sys;
...
static unsigned long sd = 0x18900u;
...
static int one(const wchar_t* s, int base){
    ...
    *sys_errno()=0; hits=0; unsigned long va = wia_wcstoul(s,&ea,base);           h_a = hits; int ra = *sys_errno();
    *sys_errno()=0;         unsigned long vb = ref_wcstoul((const unsigned short*)s,&eb,base); int rb = *sys_errno();
    *sys_errno()=0; hits=0; unsigned long vc = sys(s,&ec,base);                  h_c = hits; int rc = *sys_errno();
...
static int one_nullep(const wchar_t* s, int base){
    *sys_errno()=0; hits=0; unsigned long va = wia_wcstoul(s,NULL,base); long h_a=hits; int ra=*sys_errno();
    *sys_errno()=0; hits=0; unsigned long vc = sys(s,NULL,base);        long h_c=hits; int rc=*sys_errno();
...
int main(void){
    ...
    sys = (WL)GetProcAddress(hu,"wcstoul");
    sys_errno = (int*(__cdecl*)(void))GetProcAddress(hu,"_errno");
    if(!sys||!sys_errno){ printf("CORRECTNESS: cannot resolve ucrtbase!wcstoul/_errno\n"); return 1; }
    {
    ...
    // ---- saturation / ERANGE / no-conversion literals ----
    {
        static const wchar_t* lit[] = {
            ...
            L"-1", L"-2", L"-4294967295", L"-4294967296", L"4294967294", L"4294967295",
            L"0xfffffffe", L"0xffffffff", L"-0x1",
    ...
    for(int blk=1; blk<18; ++blk){
        static const wchar_t* lim[] = { L"2147483647", L"2147483648", L"4294967296" };
        for(int L=0;L<3;L++){
            int k=0;
    ...
    // ---- page guard ----
    {
        ...
        for(int tail=1; tail<=40; ++tail){
            for(int k=0;k<NB;k++){
                ...
                CHECK(wia_wcstoul(p,&e1,BASES[k])==sys(p,&e2,BASES[k]) && e1==e2,
            ...
            p[0]=DBLK[3]; p[1]=0;
            wchar_t *e1=SENT,*e2=SENT;
            CHECK(wia_wcstoul(p,&e1,16)==sys(p,&e2,16) && e1==e2, "page-guard: prefix probe, base 16");
            CHECK(wia_wcstoul(p,&e1,0) ==sys(p,&e2,0)  && e1==e2, "page-guard: prefix probe, base 0");
        }
        VirtualFree(base,0,MEM_RELEASE);
    }

    if(fails){ printf("CORRECTNESS: FAILED (%d)\n", fails); return 1; }
    printf("CORRECTNESS: PASS (wcstoul vs live ucrtbase + oracle -- value, *endptr, errno AND "
           ...
           "x 5 positions, 35 literals x 15 bases x {endptr, NULL}, the limits rewritten in every "
           "non-ASCII block, the modulo-2^32 negation cases, 1.5M fuzz, NOACCESS page-guard incl. the prefix probe at the edge)\n");
    return 0;
}
