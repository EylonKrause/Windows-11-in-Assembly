// changes/203-convertguidtostringa/correctness.c
// Gate 1: wia_ConvertGuidToStringA must be indistinguishable from iphlpapi!ConvertGuidToStringA --
...
int ref_ConvertGuidToStringA(const GUID*, char*, unsigned long);
...
static int one(const GUID* g, DWORD cch){
    ...
    DWORD ra = wia_ConvertGuidToStringA(g,a,cch);
    DWORD rb = (DWORD)ref_ConvertGuidToStringA(g,b,cch);
...
int main(void){
    ...
    if(!sys){ printf("CORRECTNESS: cannot resolve iphlpapi!ConvertGuidToStringA\n"); return 1; }
    ...
    // ---- NULL arguments ----
    {
        ...
        DWORD ra = wia_ConvertGuidToStringA(NULL,a,64);
        ...
        CHECK(wia_ConvertGuidToStringA(&FIXED[0],NULL,64) == sys(&FIXED[0],NULL,64),
              "NULL String");
        CHECK(wia_ConvertGuidToStringA(NULL,NULL,0) == sys(NULL,NULL,0), "both NULL, cch 0");
    }
    ...
    // ---- page guard: the output buffer ends exactly at a PAGE_NOACCESS boundary ----
    {
        ...
        static char mirror[DSZ];
        for(DWORD cch=1; cch<=64; ++cch){
            char* p = (char*)((base+pg) - (size_t)cch*2);
            for(DWORD i=0;i<cch;i++) p[i]=PW;
            DWORD ra = wia_ConvertGuidToStringA(&FIXED[0],p,cch);
        ...
        /* and the GUID itself ending at the guard page, to prove the 16-byte load is in bounds */
        {
            ...
            static char a[DSZ], c[DSZ];
            for(int i=0;i<DSZ;i++){ a[i]=PW; c[i]=PW; }
            DWORD ra = wia_ConvertGuidToStringA(gp,a,64);
    ...
    printf("CORRECTNESS: PASS (ConvertGuidToStringA vs live iphlpapi + oracle -- return AND the "
