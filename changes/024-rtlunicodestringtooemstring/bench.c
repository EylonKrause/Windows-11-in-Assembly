...
static uint64_t op_ours(void*c){ ctx_t*m=(ctx_t*)c; m->dst.Length=0; return (uint64_t)(unsigned)wia_u2oem(&m->dst,&m->src,0); }
...
int main(void){
    ...
    return wia_bench_compare("RtlUnicodeStringToOemString  (wia AVX2 vs ntdll, no-alloc)", cs, K, 200);
}
