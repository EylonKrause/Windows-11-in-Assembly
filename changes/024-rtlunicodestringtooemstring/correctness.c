// changes/024-rtlunicodestringtooemstring/correctness.c
//
// Bit-exact fuzz of wia_u2oem vs live ntdll!RtlUnicodeStringToOemString + oracle.
//
...
extern NTSTATUS wia_u2oem(ASTR*, const USTR*, unsigned char);
long ref_u2oem(void*, const void*, int);
void wia_oemmap_init(void);
...
static void one(const wchar_t* src, int n, unsigned short maxlen)
{
    ...
    yo=sys(&u1,&us,FALSE);
    oo=wia_u2oem(&u2,&us,0);
    ro=ref_u2oem((void*)&ur,(void*)&us,0);
...
int main(void){
    ...
    int n,i,k,need;
    wia_oemmap_init();
    { HMODULE h=LoadLibraryW(L"ntdll.dll"); sys=(fn)GetProcAddress(h,"RtlUnicodeStringToOemString"); }
    if(!sys){ printf("no RtlUnicodeStringToOemString\n"); return 2; }
    ...
    if(!failures)
        printf("CORRECTNESS: PASS (RtlUnicodeStringToOemString n=0..280, MaximumLength swept across the\n"
