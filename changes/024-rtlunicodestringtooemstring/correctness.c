...
long ref_u2oem(ASTR*, const USTR*, int);
void wia_oemmap_init(void);
typedef NTSTATUS (WINAPI *fn)(ASTR*,const USTR*,BOOLEAN);
static int failures=0;
int main(void){
    wia_oemmap_init();
    HMODULE h=LoadLibraryW(L"ntdll.dll"); fn sys=(fn)GetProcAddress(h,"RtlUnicodeStringToOemString");
    ...
    for(int n=0;n<=280;++n){
        ...
        long yo=sys(&u1,&us,FALSE), oo=wia_u2oem(&u2,&us,0), ro=ref_u2oem((void*)&ur,(void*)&us,0);
    ...
    { USTR us={40,40,src}; ASTR u2={0,10,d2}; if(wia_u2oem(&u2,&us,0)!=(long)0x80000005){printf("FAIL overflow\n");++failures;} }
    if(!failures) printf("CORRECTNESS: PASS (UTF-16->OEM n=0..280, ASCII+nonASCII + overflow, vs ntdll)\n");
