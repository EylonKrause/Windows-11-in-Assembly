...
#pragma optimize("", off)
static uint64_t op_ours(void*c){ ctx_t*m=(ctx_t*)c; wia_rtlinitstr(&m->us,m->s); return m->us.Length; }
static uint64_t op_sys (void*c){ ctx_t*m=(ctx_t*)c; sys(&m->us,m->s); return m->us.Length; }
#pragma optimize("", on)
int main(void){
    HMODULE h=GetModuleHandleW(L"ntdll.dll"); sys=(fn)GetProcAddress(h,"RtlInitString");
    ...
    return wia_bench_compare("RtlInitString (wia inline AVX2 strlen vs ntdll call+strlen)", cs, K, 200);
}
