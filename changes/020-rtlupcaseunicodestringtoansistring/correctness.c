...
long ref_u2au(ASTR*, const USTR*, int);
void wia_upansimap_init(void);
typedef NTSTATUS (WINAPI *fn)(ASTR*,const USTR*,BOOLEAN);
static int failures=0;
int main(void){
    wia_upansimap_init();
    HMODULE h=LoadLibraryW(L"ntdll.dll"); fn sys=(fn)GetProcAddress(h,"RtlUpcaseUnicodeStringToAnsiString");
    ...
    for(int n=0;n<=280;++n){
        ...
        ASTR u1={0,300,d1}, u2={0,300,d2}, ur={0,300,dr};
        long yo=sys(&u1,&us,FALSE), oo=wia_u2au(&u2,&us,0), ro=ref_u2au((void*)&ur,(void*)&us,0);
    ...
    if(!failures) printf("CORRECTNESS: PASS (upcase+UTF-16->ANSI n=0..280, ASCII+nonASCII, vs ntdll)\n");
