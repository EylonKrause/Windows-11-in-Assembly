...
// _strupr modifies in place; the fold is idempotent (A-Z->a-z, then a-z stays), so the
...
#pragma optimize("", off)
static uint64_t op_ours(void*c){ return (uint64_t)(uintptr_t)wia_strupr(((ctx_t*)c)->s); }
static uint64_t op_sys (void*c){ return (uint64_t)(uintptr_t)sys(((ctx_t*)c)->s); }
#pragma optimize("", on)
int main(void){
    HMODULE h=LoadLibraryW(L"ucrtbase.dll"); sys=(fn)GetProcAddress(h,"_strupr");
    ...
    return wia_bench_compare("_strupr  (wia AVX2 fold+store vs ucrtbase, in place)", cs, K, 200);
}
