// changes/200-itow-s/correctness.c
// Gate 1: wia_itow_s must be indistinguishable from ucrtbase!_itow_s -- the return value, the
...
int ref_itow_s(int, wchar_t*, size_t, int);
typedef int (__cdecl *FN)(int, wchar_t*, size_t, int);
static FN sys;
...
static unsigned long sd = 0x20000u;
...
static int one(int v, size_t size, int radix){
    ...
    long h0, h2;
    *sys_errno()=0; hits=0; int ra = wia_itow_s(v,a,size,radix); h0=hits; int ea=*sys_errno();
    *sys_errno()=0;         int rb = ref_itow_s(v,b,size,radix);          int eb=*sys_errno();
...
static int one_null(int v, size_t size, int radix){
    *sys_errno()=0; hits=0; int ra = wia_itow_s(v,NULL,size,radix); long h0=hits; int ea=*sys_errno();
...
int main(void){
    ...
    sys = (FN)GetProcAddress(hu,"_itow_s");
    sys_errno = (int*(__cdecl*)(void))GetProcAddress(hu,"_errno");
    if(!sys||!sys_errno){ printf("CORRECTNESS: cannot resolve ucrtbase!_itow_s/_errno\n"); return 1; }
    {
    ...
    // ---- _ltow_s is a SEPARATE export at a different address, laid out with the branch inverted.
    //      Check it behaves identically rather than assuming it, on the same three-way basis:
    //      return, whole buffer AND errno.
    {
        FN alias = (FN)GetProcAddress(hu,"_ltow_s");
        CHECK(alias!=NULL, "ucrtbase!_ltow_s resolves");
        if(alias){
            static wchar_t ba[DSZ], bb[DSZ];
            for(int t=0;t<200000 && fails==0;t++){
                int v = (int)(unsigned)((rnd()<<16) ^ rnd());
                int r = 2 + (int)(rnd()%35);
                if(rnd()%16==0) r = (int)(rnd()%80) - 10;
                size_t sz = rnd()%40;
                for(int i=0;i<DSZ;i++){ ba[i]=(wchar_t)PB; bb[i]=(wchar_t)PB; }
                ...
                *sys_errno()=0; int r2 = alias(v,bb,sz,r); int e2=*sys_errno();
                ...
                CHECK(ok, "_ltow_s behaves identically to _itow_s");
            }
        }
    }
    ...
    // ---- page guard: the buffer ends exactly at a PAGE_NOACCESS boundary ----
    {
        ...
        for(size_t s=1; s<=70; ++s){
            for(int r=2;r<=36;r+=7){
                ...
                *sys_errno()=0; hits=0; int ra = wia_itow_s(-1234567890,p,s,r); long h0=hits;
    ...
    printf("CORRECTNESS: PASS (_itow_s AND its _ltow_s alias vs live ucrtbase + oracle -- return, WHOLE buffer (the "
