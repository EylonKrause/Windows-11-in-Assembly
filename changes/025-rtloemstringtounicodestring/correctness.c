// changes/025-rtloemstringtounicodestring/correctness.c
//
// Bit-exact fuzz of wia_oem2u vs live ntdll!RtlOemStringToUnicodeString + oracle.
//
...
extern NTSTATUS wia_oem2u(USTR*, const ASTR*, unsigned char);
long ref_oem2u(void*, const void*, int);
...
static void one(const char* src, int n, unsigned short maxlen)
{
    ...
    yo=sys(&u1,&us,FALSE);
    oo=wia_oem2u(&u2,&us,0);
    ro=ref_oem2u((void*)&ur,(void*)&us,0);
...
int main(void){
    ...
    int n,i,k,need;
    wia_oem2umap_init();
    { HMODULE h=LoadLibraryW(L"ntdll.dll"); sys=(fn)GetProcAddress(h,"RtlOemStringToUnicodeString"); }
    if(!sys){ printf("no RtlOemStringToUnicodeString\n"); return 2; }
    ...
    if(!failures)
        printf("CORRECTNESS: PASS (RtlOemStringToUnicodeString n=0..280, MaximumLength swept across the\n"
