...
long ref_oem2u(USTR*, const ASTR*, int);
void wia_oem2umap_init(void);
typedef NTSTATUS (WINAPI *fn)(USTR*,const ASTR*,BOOLEAN);
static int failures=0;
int main(void){
    wia_oem2umap_init();
    HMODULE h=LoadLibraryW(L"ntdll.dll"); fn sys=(fn)GetProcAddress(h,"RtlOemStringToUnicodeString");
    ...
    for(int n=0;n<=280;++n){
        ...
        long yo=sys(&u1,&us,FALSE), oo=wia_oem2u(&u2,&us,0), ro=ref_oem2u((void*)&ur,(void*)&us,0);
    ...
    { ASTR us={40,40,src}; USTR u2={0,20,d2}; if(wia_oem2u(&u2,&us,0)!=(long)0x80000005){printf("FAIL overflow\n");++failures;} }
    if(!failures) printf("CORRECTNESS: PASS (OEM->UTF-16 n=0..280, ASCII+highbytes + overflow, vs ntdll)\n");
