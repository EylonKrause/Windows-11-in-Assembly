...
static uint64_t op_ours(void*c){ ctx_t*m=(ctx_t*)c; return wia_rtlequstr(&m->u1,&m->u2,m->ci); }
...
int main(void){
    wia_upcase_init();
    HMODULE h=LoadLibraryW(L"ntdll.dll"); sys=(fn)GetProcAddress(h,"RtlEqualUnicodeString");
    ...
    return wia_bench_compare("RtlEqualUnicodeString  (wia AVX2 vs ntdll; cs + CI)", cs, 2*K, 200);
}
