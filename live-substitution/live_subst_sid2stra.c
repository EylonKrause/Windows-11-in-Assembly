...
typedef BOOL (WINAPI *F_S2S)(PSID, LPSTR*);

extern BOOL wia_sid2stra(const void*, char**);

static volatile LONG c_hit;
static BOOL WINAPI w_s2s(PSID s, LPSTR* p)
{ _InterlockedIncrement(&c_hit); return wia_sid2stra((const void*)s, (char**)p); }
...
#define NCASE 40000
#define POISON ((LPSTR)(UINT_PTR)0xDEADBEEFDEADBEEFull)
...
int main(void)
{
    ...
    live = (F_S2S)GetProcAddress(ha, "ConvertSidToStringSidA");
    if (!live) { printf("resolve failed\n"); return 1; }

    setvbuf(stdout, NULL, _IONBF, 0);
    printf("== LIVE SUBSTITUTION: advapi32!ConvertSidToStringSidA (change 271) ==\n");
    {
        HMODULE owner = 0;
    ...
    printf("  [pre-patch]  %d cases recorded from the SHIPPED export;  TRUE %ld (of which %ld gave a\n"
           "               block over 100 bytes -- six full sixteen-character pack runs -- and %ld\n"
           "               took the HEXADECIMAL identifier authority), ERROR_INVALID_SID %ld\n",
    ...
    OK(n_long > 1000,  "the corpus rarely produced a block long enough for the sixteen-character\n"
                       "        pack loop to run more than a few times");
