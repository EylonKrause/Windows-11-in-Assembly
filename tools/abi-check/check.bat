...
rem  284 (StrStrIW) links change 281's generated match-relation tables unchanged.
rem  285 (StrCSpnIW) links change 281's generated match-relation tables unchanged, which
rem  probes/relation.c verified over 786420 pairs rather than assuming.
rem  286 (StrChrNIW) links change 281's generated match-relation tables unchanged.
rem  287 (GetStringTypeW) derives its three classification tables from the live export at
rem  init and re-checks every entry through both layouts, so it needs its own tables.c.
rem  288 (FoldStringW MAP_FOLDDIGITS) derives its fold table from the live export at init.
if "%ID%"=="288" (
  cl /nologo /O2 /c /Fo"%H%dep288t.obj" "%H%..\..\changes\288-foldstringw-digits\tables.c" >nul 2>&1
  set EXTRA=!EXTRA! "%H%dep288t.obj"
)
if "%ID%"=="287" (
  cl /nologo /O2 /c /Fo"%H%dep287t.obj" "%H%..\..\changes\287-getstringtypew\tables.c" >nul 2>&1
  set EXTRA=!EXTRA! "%H%dep287t.obj"
)
if "%ID%"=="286" (
