; ucrtbase.dll!_strlwr_s  --  hand-written x86-64 reimplementation (5.92x vs shipped)
; source of truth: changes/179-strlwr-s/  (reference.c + correctness.c + bench.c)
; validated bit-exact vs the live export; see that dir's RESULTS.md.
;----------------------------------------------------------------------
; changes/179-strlwr-s/impl.asm
; errno_t wia_strlwr_s(char* str, size_t numberOfElements)   [Win64: rcx, rdx -> eax]
;
; Reimplements ucrtbase!_strlwr_s -- the byte sibling of change 178 (_wcsupr_s) and the bounded
; form of change 047 (_strlwr). ucrtbase's is scalar: 72.8 ns for a 254-byte string.
;
...
; folds a KNOWN length, so it needs no terminator test -- change 047's fold (two vpcmpgtb form
...
.const
ALIGN 16
c40b db 32 dup(40h)                      ; 'A' - 1
c5Bb db 32 dup(5Bh)                      ; 'Z' + 1
...
fold:
        ...
        vpcmpgtb  ymm2, ymm0, ymmword ptr [c40b]    ; c > 'A'-1  (signed: >= 0x80 never matches)
