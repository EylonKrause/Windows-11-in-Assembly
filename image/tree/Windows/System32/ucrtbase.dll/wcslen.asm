; ucrtbase.dll!wcslen  --  hand-written x86-64 reimplementation (2.15x vs shipped)
; source of truth: changes/001-wcslen/  (reference.c + correctness.c + bench.c)
; validated bit-exact vs the live export; see that dir's RESULTS.md.
;----------------------------------------------------------------------
; changes/001-wcslen/impl.asm
; size_t wia_wcslen(const wchar_t* s)   [Win64: rcx -> rax]
;
...
; blocks (two YMM loads combined with vpminuw), and a 64-aligned 64-byte block
...
wia_wcslen PROC
        ...
        vpcmpeqw  ymm0, ymm1, ymmword ptr [r8]   ; scan one 32B block to reach 64-alignment
        vpmovmskb edx, ymm0
        test      edx, edx
        jnz       found_at_r8
...
loop64:
        ...
        vpminuw   ymm2, ymm0, ymmword ptr [r8+32]; per-lane min(v0, v1): 0 iff either had 0
...
found_at_r8:
        ...
        lea       r10, [r8 + rdx]
finish:
        sub       r10, rax                       ; bytes from start to terminator
        mov       rax, r10
