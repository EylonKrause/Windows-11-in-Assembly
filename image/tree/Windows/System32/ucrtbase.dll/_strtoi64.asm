; ucrtbase.dll!_strtoi64  --  hand-written x86-64 reimplementation (1.79x vs shipped)
; source of truth: changes/112-strtoi64/  (reference.c + correctness.c + bench.c)
; validated bit-exact vs the live export; see that dir's RESULTS.md.
;----------------------------------------------------------------------
; changes/112-strtoi64/impl.asm
; __int64 wia_strtoi64(const char* nptr, char** endptr, int base)   [rcx, rdx, r8d -> rax]
;
; ucrtbase!_strtoi64: the 64-bit signed strtol. Same parse as strtol (skip C-locale whitespace, one
; optional sign, base 0/2..36 with the "0x"/"0X" prefix and base-0 auto-detect, endptr = first
; unparsed char). Overflow saturates: positive > _I64_MAX (0x7FFFFFFFFFFFFFFF) -> _I64_MAX, negative
; magnitude > 2^63 -> _I64_MIN (0x8000000000000000), both with errno = ERANGE (34). ucrtbase routes it
; through the locale-aware CRT (~13-22 ns); this is a scalar loop with a 256-entry digit table and a
...
no_ep:
        ...
        test      r13d, r13d
        jz        epilogue
        neg       rax                                 ; -magnitude (2^63 -> _I64_MIN)
        jmp       epilogue
do_overflow:
        sub       rsp, 20h
        call      _errno
