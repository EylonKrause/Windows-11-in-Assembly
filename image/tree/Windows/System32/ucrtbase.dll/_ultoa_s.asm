; ucrtbase.dll!_ultoa_s  --  hand-written x86-64 reimplementation (1.77x vs shipped)
; source of truth: changes/199-ultoa-s/  (reference.c + correctness.c + bench.c)
; validated bit-exact vs the live export; see that dir's RESULTS.md.
;----------------------------------------------------------------------
; changes/199-ultoa-s/impl.asm
; errno_t wia_ultoa_s(unsigned long Value, char* Buffer, size_t SizeInChars, int Radix)
;   [rcx, rdx, r8, r9d -> eax]
;
; Reimplements ucrtbase!_ultoa_s -- the unsigned 32-bit form, and the bounded form of change 054.
; Its entry at 0x00035870 passes a HARD ZERO for `negative` into the same worker the signed entry
; calls, so this is change 198 with the sign permanently absent and the early ERANGE test reduced
; to SizeInChars <= 1.
;
...
wia_ultoa_s PROC
        ...
        xor       r12d, r12d                   ; negative is permanently 0 for the unsigned form,
                                               ; the hard zero ucrtbase's unsigned entry passes to
                                               ; the worker its signed entry shares
        lea       rax, [r12 + 1]
        cmp       rsi, rax
