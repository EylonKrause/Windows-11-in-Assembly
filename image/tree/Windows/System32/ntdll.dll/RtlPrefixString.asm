; ntdll.dll!RtlPrefixString  --  hand-written x86-64 reimplementation (2.88x vs shipped)
; source of truth: changes/014-rtlprefixstring/  (reference.c + correctness.c + bench.c)
; validated bit-exact vs the live export; see that dir's RESULTS.md.
;----------------------------------------------------------------------
; changes/014-rtlprefixstring/impl.asm
; BOOLEAN wia_rtlprefix_a(const STRING* s1, const STRING* s2, BOOLEAN ci)  [rcx, rdx, r8b -> al]
;
; Reimplements ntdll!RtlPrefixString (ANSI/8-bit): TRUE iff s1 is a prefix of
; s2, i.e. Length1 <= Length2 and the first Length1 bytes match (upcased if ci). CS: 32-byte vpcmpeqb, overlapping tail. CI: 8-byte
...
.code
wia_rtlprefix_a PROC
        push      r13
        movzx     r9d, word ptr [rcx]              ; Length1 (prefix length) = n
        movzx     r10d, word ptr [rdx]             ; Length2
        cmp       r9d, r10d
        ja        ret_false                        ; L1 > L2 -> not a prefix
