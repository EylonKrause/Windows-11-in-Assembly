; ntdll.dll!RtlUnicodeToUTF8N  --  hand-written x86-64 reimplementation (2.46x vs shipped)
...
; -------------------------------------------------------------------------------------------------
; The shift table, and the defect it exists to fix.
;
...
; only branch. Two stores and a shuffle, no read, no stall.
; -------------------------------------------------------------------------------------------------
ALIGN 16
SHIFTR LABEL BYTE
PKI = 0
...
wia_u2u8_shift_table PROC
        lea       rax, [SHIFTR]
        ret
wia_u2u8_shift_table ENDP
...
lat8:
        ...
        add       r15, rax                          ; fewer than eight, so the branch is settled
        add       r14, 8
        jmp       mainloop
; -------------------------------------------------------------------------------------------------
...
bmp8:
        ...
        STOREX    xmm3, xmm1                        ; BOTH halves store exactly. The second half
        add       r15, rax                          ; begins where this one's surplus would begin,
                                                    ; so it looked as though it would cover it --
                                                    ; but it writes only its OWN length too, and a
                                                    ; surplus zero left here would simply survive.
